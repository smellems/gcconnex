...
/**
 * Elgg GSA plugin
 * ssh -R 9000:localhost:9000 ctp@ctp3.ctpartners.ca -p 2222
 */

elgg_register_event_handler('init','system','gsa_init');

/**
 * Initialize GSA plugin
 */
function gsa_init() {
    global $CONFIG;
    ...
    elgg_register_page_handler('gsa', 'search_page_handler');

    // register some default search hooks
    elgg_register_plugin_hook_handler('gsa', 'object', 'search_objects_hook');
    elgg_register_plugin_hook_handler('gsa', 'user', 'search_users_hook');
    elgg_register_plugin_hook_handler('gsa', 'group', 'search_groups_hook');
    ...
    elgg_register_plugin_hook_handler('gsa', 'tags', 'search_tags_hook');

    elgg_register_plugin_hook_handler('search_types', 'get_types', 'search_custom_types_comments_hook');
    elgg_register_plugin_hook_handler('gsa', 'comments', 'search_comments_hook');
...
//    }
    ...
    elgg_extend_view('css/elgg', 'gsa/css');

    // extend view for elgg topbar search box
    elgg_extend_view('page/elements/header', 'gsa/header');
}
...
function search_page_handler($page) {
    ...
    $base_dir = elgg_get_plugins_path() . 'gsa/pages/gsa';
 ...
 * @return string
 */
function search_get_highlighted_relevant_substrings($haystack,
                                                    $query,
                                                    $min_match_context = 30,
                                                    $max_length = 300,
                                                    $tag_match = false) {
 ...
 *
 * @param array $query
 * @param \str|string $format Return as an array or a string
 * @return mixed
 */
...
function search_get_search_view($params, $view_type) {
    ...
    if (isset($params['type']) && $params['type'] && isset($params['subtype']) && $params['subtype']) {
        $view_order[] = "gsa/{$params['type']}/{$params['subtype']}/$view_type";
    }

    // also check for the default type
    if (isset($params['type']) && $params['type']) {
        $view_order[] = "gsa/{$params['type']}/$view_type";
    }

    // check search types
    if (isset($params['search_type']) && $params['search_type']) {
        $view_order[] = "gsa/{$params['search_type']}/$view_type";
    }

    // finally default to a search list default
    $view_order[] = "gsa/$view_type";
