...
if ($entity->getVolatileData('search_unavailable_entity')) {
	$title = elgg_echo('gsa:comment_on', array(elgg_echo('gsa:unavailable_entity')));
} else {
	...
	$title = elgg_echo('gsa:comment_on', array($title));
	$title .= ' ' . elgg_echo('gsa:comment_by') . ' ' . $author_name;
