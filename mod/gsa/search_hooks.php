 ...
 * @param $gsa_xml
 * @return mixed php array structure to iterate when displaying results
 */
...
//                    $result['crawldate'] = (string) $res->FS['VALUE'];
                    $result['crawldate'] = isset($res['CRAWLDATE']) ? (string) $res['CRAWLDATE'] : '';
                    ...
                    $url_path = parse_url($result['abs_url']);
                    $segments = explode('/', $url_path['path']);
                    switch($segments[1]) {
                        case 'photos':
                        case 'blog':
                        case 'bookmark':
                        ...
                        case 'task_top':
                        case 'thewire':
                            $result['entity_type'] = 'object';
                            $result['sub_type'] = $segments[2];
                            if ($segments[2] == 'view') {
                                $result['sub_type'] = $segments[1];
                            }
...
/**
 * Get objects that match the gsa search parameters.
 *
