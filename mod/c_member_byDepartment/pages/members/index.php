...
function render_department_tab($data_directory) {
	...
	{	
		...
		{
			$query = "SELECT ue.email, ue.username, ue.guid, e.time_created, ue.name FROM elggentities e, elggusers_entity ue WHERE e.type = 'user' AND e.guid = ue.guid AND e.enabled = 'yes' AND e.guid = ".$key;
			$connection = mysqli_connect($CONFIG->dbhost, $CONFIG->dbuser, $CONFIG->dbpass, $CONFIG->dbname);
			if (mysqli_connect_errno($connection)) elgg_log("cyu - Failed to connect to MySQL: ".mysqli_connect_errno(), 'NOTICE');
			$result = mysqli_query($connection,$query);
			mysqli_close($connection);
			...
			while ($row = mysqli_fetch_array($result))
			{
				if ($row['email'] === 'charles.degrasse@tbs-sct.gc.ca' || $row['email'] === 'christineyu@tbs-sct.gc.ca')
				{
					$user = get_user($key);
					$tmp_arr = array_merge($main_json_file[$value], array($key => strtolower($row['email']).'|'.strtolower($row['username']).'|'.$key.'|'.$row['time_created'].'|'.$row['name'].'|'.$user->getIconURL()));
				} else
					$tmp_arr = array_merge($main_json_file[$value], array($key => strtolower($row['email']).'|'.strtolower($row['username']).'|'.$key.'|'.$row['time_created'].'|'.$row['name'].'|'.'http://192.168.0.100/elgg-prod2//mod/gc_glee_theme/graphics/flag.gif'));
				$main_json_file[$value] = $tmp_arr;
			}
		}
