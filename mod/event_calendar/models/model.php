...
function event_calendar_format_time($date,$time1,$time2='') {
	if (is_numeric($time1)) {
		...
		if (strpos($_SERVER['REQUEST_URI'],'event_calendar/list'))
...
function event_calendar_generate_listing_params($page_type,$container_guid,$original_start_date,$display_mode,$filter,$region='-') {
	...
	error_log('cyu - s_tme:'.$start_ts.' +++ e_tme:'.$end_ts);
	...
	error_log('cyu - subtitle:'.$subtitle.' +++ title:'.$title);
	error_log('cyu - orig:'.$original_start_date.' +++ sd:'.$start_date.' +++ ed:'.$end_date.' +++ evt_fd:'.$event_calendar_first_date.' +++ evt_ld'.$event_calendar_last_date);
	//error_log('cyu - s_tme:'.$start_ts.' +++ e_tme:'.$end_ts);
	return $params;
}
