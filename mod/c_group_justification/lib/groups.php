			...
			if ($text === 'groups:joinrequest')
			{
				...
				if (count($resultset) > 0)
				{
					foreach ($resultset as $result)
					{
						...
						if ($user_group_info[1] == get_loggedin_userid() && $user_group_info[2] == $group->getGUID())
						{
							$found = true;
						}
					}
					...
					if (!$found)
					{
						$group_guid = explode('/', $url);
						elgg_register_menu_item('title', array(
							'name' => 'test',
							'href' => 'c_group_justification/show_join_application?grp_id='.$group_guid[6].'&send_app_url='.$url,
							'text' => elgg_echo('grp_just:apply'),
						...
						elgg_register_menu_item('title', array(
							'name' => 'test2',
							'text' => elgg_echo('grp_just:app_sent'),
							'link_class' => 'deactivate_this_link',
						));
					}

				// not groups:joinrequest
				} else {
					elgg_register_menu_item('title', array(
						'name' => $text,
						'href' => $url,
