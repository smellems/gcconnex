...
/**
 * 
 * ATTEMPTING IN JOINING A CLOSED GROUP
 *
 */

$justification_text = get_input('justification');
$mInfo = get_input('member_info');
$gInfo = get_input('group_info');
...
if (($user instanceof ElggUser) && ($group instanceof ElggGroup)) {
	...
	if ($join) {
		...
		$c_obj->subtype = 'c_group_reason';
		$c_obj->title = $mInfo.';'.$gInfo;
		$c_obj->description = $justification_text;
		$c_obj->save();
		$c_obj->type = 'object';
		$c_obj->processed = false;


		// notification to user that the request is sent successfully
		if (notify_user($group->owner_guid, $user->getGUID(), $subject, $body)) {
			system_message(elgg_echo("groups:joinrequestmade"));
			system_message(elgg_echo('grp_just:app_sent'));
		} else {
			register_error(elgg_echo("groups:joinrequestnotmade"));
		}
		forward(elgg_get_site_entity().'groups/profile/'.$group->guid.'/'.$group->title);
	}
} else {
	register_error(elgg_echo("groups:cantjoin"));
}