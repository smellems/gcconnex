...
if (!empty($vars['requests']) && is_array($vars['requests'])) {
	...
	foreach ($vars['requests'] as $user) {
		...
		$rev = elgg_view('output/url', array(
			'href' => 'c_group_justification/show_reason?userid='.$user->guid.'&groupid='.$vars['entity']->guid, 
			'class' => 'elgg-lightbox',
			'text' => elgg_echo('grp_just:review_app'),
		));

		$body = "<h4>$user_title | $rev </h4>";
