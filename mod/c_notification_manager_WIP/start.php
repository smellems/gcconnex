...
{
	elgg_log('cyu - notify handler invoked from: '.$_SERVER['HTTP_REFERER'], 'NOTICE');
	if (strstr($_SERVER['HTTP_REFERER'], 'group') || strstr($_SERVER['HTTP_REFERER'], 'discussion'))
	{
		elgg_log('cyu - $$$$$$$$$$ this is a group notification :-) $$$$$$$$$$', 'NOTICE');
	}

	elgg_log('cyu - from:'.$from->type, 'NOTICE');
	...
	{
		elgg_log('cyu - method:'.$method.'  +++  foo:'.$foo, 'NOTICE');
	}
...
{
	elgg_log('cyu - object notification invoked', 'NOTICE');
