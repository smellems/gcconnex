<?php
	...
	function advanced_notifications_is_registered_notification_entity(ElggEntity $entity, $subject = false){
		elgg_log('cyu - notification is registered entity', 'NOTICE');
	...
	*/
	function advanced_notifications_is_registered_notification_annotation(ElggAnnotation $annotation){
		elgg_log('cyu - notification is registered annotation', 'NOTICE');
		$result = false;
	...
	function advanced_notifications_start_commandline($options = array()){
		elgg_log('cyu - start commandline', 'NOTICE');
	...
	function advanced_notifications_entity_notification($guid, $event){
		elgg_log('cyu - entity notification sent out', 'NOTICE');
										...
										if($user = get_user($user_guid)){
											...
											if(has_access_to_entity($entity, $user)){
												...
												if($message !== false){
													...
													// put it in the database and send later :-P
	...
	function advanced_notifications_annotation_notification($id, $event){
		elgg_log('cyu - annotation notification is sent out', 'NOTICE');
		global $NOTIFICATION_HANDLERS;
											...
											foreach($user_guids as $user_guid){
												...
												if($user = get_user($user_guid)){
													...
													if(has_access_to_entity($entity, $user)){
														...
														// if the hook returnd false, don't sent a notification
														if($message !== false){
															elgg_log('cyu - sending in site message to...'.$user->getGUID(), 'NOTICE');
															elgg_log('cyu - sending in site with the subject...'.$subject, 'NOTICE');
															elgg_log('cyu - sending in site with the message...'.$message, 'NOTICE');

															// put it into the database... send later :-P
															notify_user($user->getGUID(), $entity->getContainerGUID(), $subject, $message, null, $method);
														}
													}
												}
