...
/**
 *	js part of the AJAX plugin, much of it from MediaWiki
 *	
 */
 
 window.elggDoAjax = function(func_name, args, target) {
	var i, x, n;
	var uri;
	//var post_data;
	uri = elgg.config.wwwroot;
	//if ( sajax_request_type == 'GET' ) {
		if ( uri.indexOf( '?' ) == -1 ) {
			uri = uri + '?AJAXfunc=' + encodeURIComponent( func_name );
		} else {
			uri = uri + '&AJAXfunc=' + encodeURIComponent( func_name );
		}
