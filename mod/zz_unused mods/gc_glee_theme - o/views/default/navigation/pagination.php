...
$offset = abs((int) get_input( 'offset', get_input( 'annoff', 0 ) ));
//$offset = abs( (int)get_input('offset', 0) ); // abs((int) elgg_extract('offset', $vars, 0));
...
foreach ($pages->items as $page) {
	if ($page == $current_page) {
		...
		// GCPEDIA EDIT: generate link through elgg_view like everything else
		$linkdata = array (
			'text' => $page,
			'href' => $url,
			'is_trusted' => true, );
		$link = elgg_view( 'output/url', $linkdata );
