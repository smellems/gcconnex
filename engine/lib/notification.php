...
{
	//elgg_log('cyu - [notification.php] email notify handler invoked', 'NOTICE');
	global $CONFIG;
	...
	//elgg_log('cyu - [notification.php][to:'.$to.'][from:'.$from.'][subject:'.$subject.'][message:'.$message.']', 'NOTICE');
	elgg_log('cyu - send mail invoked', 'NOTICE');
...
function object_notifications($event, $object_type, $object) {
	...
	//elgg_log('cyu - [notification.php] object notification invoked', 'NOTICE');
			...
			foreach ($NOTIFICATION_HANDLERS as $method => $foo) {
				...
				if ($interested_users && is_array($interested_users)) {
					foreach ($interested_users as $user) {
						...
						elgg_log('cyu - [notification.php] interested users:'.$user->username, 'NOTICE');
						...
						if ($user instanceof ElggUser && !$user->isBanned()) {
							...
							if (($user->guid != $SESSION['user']->guid) && has_access_to_entity($object, $user) && $object->access_id != ACCESS_PRIVATE) {
								...
								if ($body !== false) {
									notify_user($user->guid, $object->container_guid, $subject, $body, null, array($method));
								}
							}
