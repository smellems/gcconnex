...
*/
$(function(){
	var flagtoreplaceltngt=false;
	$('input[type="submit"]').click(function(e){
		if(flagtoreplaceltngt===true){
			flagtoreplaceltngt=false;
			return;
		}
		e.preventDefault();
		$.each($('input[type="text"],textarea'),function(i,j){
			$(j).val().replace(/</g,'CC_LT').replace(/>/g,'CC_GT');
		});
		flagtoreplaceltngt=true;
		$(this).trigger('click');
	});
});
(function($){   
